...
export const flattenObj = (obj: Record<string, any>, name?: string) => {
  const newObj: Record<string, any> = {};
  for (let k in obj) {
    if (Object(obj[k]) === obj[k]) {
      Object.assign(newObj, flattenObj(obj[k], k));
      continue;
    }
    newObj[name ? `${name}-${k}` : k] = obj[k];
  }
  return newObj;
};

export const getDataValue = (row: any, dataIndex: any) => {
  if (!dataIndex || !row) return undefined;
  if (Array.isArray(dataIndex)) {
    const key = dataIndex.join("-");
    const newRow = flattenObj(row);
    return newRow[key];
  }
