...
export type columnGetter<TRecord extends Record<any, any> = any> = (
  index: number
) => ColumnType<TRecord>;
...
const defaultFixedRowLeftColumnsClassName =
  "fixed-virtial-grid-row-left-columns";
const defaultFixedRowRightColumnsClassName =
  "fixed-virtial-grid-row-right-columns";
...
export interface IGrid<RecordType extends Record<any, any> = any>
  extends Omit<VariableSizeGrid<readonly RecordType[]>, "props" | "state"> {
...
> extends VariableSizeGrid<RecordType> {
  ...
  private _lastFixedRenderedContent:
    | Record<number | string, [React.ReactElement[], React.ReactElement[]]>
    | undefined;
  ...
  _renderFixedColumns(
    ...
    // @ts-ignore
    const { rerenderFixedColumnOnHorizontalScroll, columnWidth, rowHeight } =
      this.props;
  ...
  render() {
    ...
    const rowsColumns: Record<
      number | string,
      Record<number | string, React.ReactElement>
    > = {};
    const rowsElementProps: Record<
      number | string,
      React.HTMLAttributes<HTMLDivElement> & { key: string }
    > = {};
    ...
    if (columnCount > 0 && rowCount > 0) {
      ...
      for (let rowIndex = rowStartIndex; rowIndex <= rowStopIndex; rowIndex++) {
        ...
        const divProps = onRow ? onRow(record, rowIndex) : undefined;
        ...
        // @ts-ignore
        const firstItemStyle = this._getItemStyle(
          rowIndex,
          this._firstUnFixedColumn
        );
        ...
        for (
          ...
          if (
            columnIndex < firstUnFixedColumn ||
    ...
    const rows = Object.entries(rowsElementProps).map(([rowIndex, props]) => {
      ...
      const rowFixedColumns = rowsFixedColumns
        ? rowsFixedColumns[rowIndex]
        : [];
      const fixedLeftColumns =
        rowFixedColumns && rowFixedColumns.length ? rowFixedColumns[0] : [];
      const fixedRightColumns =
        rowFixedColumns && rowFixedColumns.length ? rowFixedColumns[1] : [];
  ...
  // @ts-ignore
  componentDidUpdate(
    prevProps: IGridProps<RecordType>,
    prevState: IGridState,
    snapshot: any
  ): void {
    if (
      // @ts-ignore
      prevProps.columnGetter !== this.props.columnGetter ||
